...
class WOE_enc(BaseEstimator, TransformerMixin):
    ...
    def fit(self, df, cols, target):
        woe_dics = {}
        for col in cols:
            woe_dics[col]={}
            woe, iv = self._calWOE(df, col, target)
            woe_dics[col]['woe'] = woe
            woe_dics[col]['iv'] = iv
        return woe_dics
        
    def transform(self, x, cols, woe_dics):
        df = pd.DataFrame()
        for col in cols:
            if col not in woe_dics:
                raise Exception('col:{} not in woe_dics'.format(col))
            df[col] = x[col].map(woe_dics[col]['woe'])
        return df

    def _calWOE(self, df ,var ,target):
        '''
        计算WOE编码
...
class ChiMerge(BaseEstimator, TransformerMixin):
    def __init__(self, null_value, max_groups=None, threshold=None): # no *args and **kwargs
    ...
    def transform(self, X, cols_dic, y=None):
        df = pd.DataFrame()
        for col, cutoffs in cols_dic.items():
...
if __name__ == '__main__':
    ...
    cm = ChiMerge(null_value=-999)
    aa = cm.fit(Train, cols=['basicLevel', 'age'], target='target')
    bb_tra = cm.transform(Train, cols_dic=aa)
    bb_val = cm.transform(Valid, cols_dic=aa)
    
    woe = WOE_enc()
    bb_tra['target']=Train.target
    a = woe.fit(bb_tra, cols=['basicLevel', 'age'], target='target')
    b = woe.transform(bb_val, cols=['basicLevel', 'age'], woe_dics=a)